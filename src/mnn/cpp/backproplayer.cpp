                    ...
                    float m, float alpha)
{
    ...
    // gradc = alpha * prev_p^T x dl/dz_l
    // gradb = (1 - alpha) * v1^T x dl/dz_l
    for(int i = 0; i < prev_p.size(); i++) {
        for(int j = 0; j < incoming.size(); j++) {
            gradc[i][j] = alpha * prev_p[i] * incoming[j];      // dL/dC_l
            gradb[i][j] = (1.0f - alpha) * incoming[j];         // dL/dB_l
            if(std::isnan(gradc[i][j])) gradc[i][j] = 0.0f;
            if(std::isnan(gradb[i][j])) gradb[i][j] = 0.0f;
            if(std::isinf(gradc[i][j])) gradc[i][j] = 1.0f;
            if(std::isinf(gradb[i][j])) gradb[i][j] = 1.0f;
        }
    }
}
                    ...
                    float m, float alpha)
{
    ...
    for (size_t i = 0; i < prev_p.size(); ++i) {
        std::transform(prevAct[i].begin(), prevAct[i].end(), dprev_p[i].begin(),
                    [&m](float x) {
                        float result = m * std::pow(x, m - 1.0f);
                        // Check for NaN or infinity
                        if (std::isnan(result) || std::isinf(result)) {
    ...
    std::vector<std::vector<float>> dprevAct = reshape(softmaxDer(flatten(dotProds), SOFTMAX_TEMP), dotProds.size(), dotProds[0].size());
...
//// Batch Backprop -> last to second layer ////
                    ...
                    float m, float alpha)
{
    ...
    for(int b = 0; b < batchSize; ++b) {
        ...
        for (size_t i = 0; i < prevAct[b].size(); ++i) { // Iterate over rows of the current batch item
            std::transform(prevAct[b][i].begin(), prevAct[b][i].end(), dprev_p[i].begin(), // Access the specific row
                        [&m](float x) {
                            float result = m * std::pow(x, m - 1.0f);
                            if (std::isnan(result) || std::isinf(result)) return 0.0f;
                            return result;
        ...
        std::vector<std::vector<float>> dprevAct = reshape(softmaxDer(flatten(dotProds[b]), SOFTMAX_TEMP), dotProds[b].size(), dotProds[b][0].size());
