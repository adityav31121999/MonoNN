...
void mnn::clBackprop(const std::vector<std::vector<float>>& expected) {
    try {
        ...
        cl::Kernel kernelSigmoidDer, kernelSub, kernelDPow, kernelvxv2m, kernelvxv2v, kernelUpdateWeightsElasticNet, kernelHadamard, kernelTranspose, kernelAvg;
        ...
        for(int i = 0; i < this->layers; i++) {
            ...
            for(int j = 0; j < batchSize; j++) {
                d_activate[i][j] = cl::Buffer(clContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * activateBatch[i][j].size(), (void*)activateBatch[i][j].data(), &err); CL_CHECK(err);
                d_dotProds[i][j] = cl::Buffer(clContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * dotProdsBatch[i][j].size(), (void*)dotProdsBatch[i][j].data(), &err); CL_CHECK(err);
                d_incoming[i][j] = cl::Buffer(clContext, CL_MEM_READ_WRITE, sizeof(float) * activateBatch[i][j].size()); CL_CHECK(err);
            }
        }
        for(int i = 0; i < this->layers-1; i++) {
            size_t actSize = activateBatch[i].size() * activateBatch[i][0].size();
        ...
        for(int layer = layers - 1; layers <= 1; --layer) {
            ...
            cl::NDRange globalOutGrad = calculate_global_1d(WORKSIZE_1D, activateBatch[layer - 1][0].size());
            ...
            for(int i = 0; i < batchSize; ++i) {
                ...
                cl::Buffer ones;
                std::vector<float> v1(activateBatch[layer - 1][i].size(), 1.0f);
                ...
                kernelvxv2v.setArg(3, (int)activateBatch[layer - 1][i].size());
                ...
                kernelDPow.setArg(3, (int)activateBatch[layer - 1][i].size());
                CL_CHECK(clCommandQueue.enqueueNDRangeKernel(kernelvxv2v, cl::NullRange, globalOutGrad, local_1d));
                // derivative of activation
                kernelSigmoidDer.setArg(0, d_dotProds[layer - 1]);
                kernelSigmoidDer.setArg(1, d_dact[layer - 1]);
                kernelSigmoidDer.setArg(2, (int)dotProdsBatch[layer - 1][i].size());
                ...
                kernelHadamard.setArg(4, (int)activateBatch[layer - 1][i].size());
            ...
            // Update C weights
            kernelUpdateWeightsElasticNet.setArg(0, d_cweights[layer]);
            kernelUpdateWeightsElasticNet.setArg(1, d_gradC[layer]);
            kernelUpdateWeightsElasticNet.setArg(2, (int)cweights[layer].size() );
            kernelUpdateWeightsElasticNet.setArg(3, (int)input.size());
            kernelUpdateWeightsElasticNet.setArg(4, learningRate);
            kernelUpdateWeightsElasticNet.setArg(5, LAMBDA_L1);
            kernelUpdateWeightsElasticNet.setArg(6, LAMBDA_L2);
            CL_CHECK(clCommandQueue.enqueueNDRangeKernel(kernelUpdateWeightsElasticNet, cl::NullRange, globalWeightGrad, local_1d));
            // update B weights
            kernelUpdateWeightsElasticNet.setArg(0, d_bweights[layer]);
            kernelUpdateWeightsElasticNet.setArg(1, d_gradB[layer]);
            kernelUpdateWeightsElasticNet.setArg(2, (int)cweights[layers].size());
            kernelUpdateWeightsElasticNet.setArg(3, (int)input.size());
            kernelUpdateWeightsElasticNet.setArg(4, learningRate);
            kernelUpdateWeightsElasticNet.setArg(5, LAMBDA_L1);
            kernelUpdateWeightsElasticNet.setArg(6, LAMBDA_L2);
        ...
        cl::NDRange globalOutGrad = calculate_global_1d(WORKSIZE_1D, activateBatch[0][0].size());
        ...
        // Update C weights
        kernelUpdateWeightsElasticNet.setArg(0, d_cweights[0]);
        kernelUpdateWeightsElasticNet.setArg(1, d_gradC[0]);
        kernelUpdateWeightsElasticNet.setArg(2, (int)cweights[0].size());
        kernelUpdateWeightsElasticNet.setArg(3, (int)input.size());
        kernelUpdateWeightsElasticNet.setArg(4, learningRate);
        kernelUpdateWeightsElasticNet.setArg(5, LAMBDA_L1);
        kernelUpdateWeightsElasticNet.setArg(6, LAMBDA_L2);
        ...
        // update B weights
        kernelUpdateWeightsElasticNet.setArg(0, d_bweights[0]);
        kernelUpdateWeightsElasticNet.setArg(1, d_gradB[0]);
        kernelUpdateWeightsElasticNet.setArg(2, (int)cweights[0].size());
        kernelUpdateWeightsElasticNet.setArg(3, (int)input.size());
        kernelUpdateWeightsElasticNet.setArg(4, learningRate);
        kernelUpdateWeightsElasticNet.setArg(5, LAMBDA_L1);
...
void mnn2d::clBackprop(const std::vector<std::vector<float>>& expected) {
    try {
        ...
        for(int i = 0; i < this->layers-1; i++) {
            size_t actSize = activateBatch[i][0].size() * activateBatch[i][0][0].size();
            d_preoutgoing[i] = std::vector<cl::Buffer>(batchSize, cl::Buffer(clContext, CL_MEM_READ_WRITE, sizeof(float) * actSize)); CL_CHECK(err);
            d_outgoing[i] = std::vector<cl::Buffer>(batchSize, cl::Buffer(clContext, CL_MEM_READ_WRITE, sizeof(float) * actSize)); CL_CHECK(err);
            d_dpow[i] = std::vector<cl::Buffer>(batchSize, cl::Buffer(clContext, CL_MEM_READ_WRITE, sizeof(float) * actSize)); CL_CHECK(err);
            d_dact[i] = std::vector<cl::Buffer>(batchSize, cl::Buffer(clContext, CL_MEM_READ_WRITE, sizeof(float) * actSize)); CL_CHECK(err);
        ...
        for (int i = 0; i < batchSize; i++) {
            ...
            for(size_t i = 0; i < activateBatch[layers-1][i].size(); ++i) {
                for(size_t j = 0; j < activateBatch[layers-1][i][0].size(); ++j) {
        ...
        for(int layer = layers - 1; layer >= 1; layer--) {
            ...
            int prev_rows = activateBatch[layer-1][0].size();
            int prev_cols = activateBatch[layer-1][0][0].size();
            int curr_rows = activateBatch[layer][0].size();
            int curr_cols = activateBatch[layer][0][0].size();
            ...
            kernelUpdate.setArg(2, curr_cols * prev_cols);
            kernelUpdate.setArg(3, learningRate);
            kernelUpdate.setArg(4, LAMBDA_L1);
            kernelUpdate.setArg(5, LAMBDA_L2);
            ...
            kernelUpdate.setArg(1, d_gradB[layer]);
            CL_CHECK(clCommandQueue.enqueueNDRangeKernel(kernelUpdate, cl::NullRange, global_update, local_2d));
        }
        ...
        kernelUpdate.setArg(0, d_cweights[0]);
        ...
        kernelUpdate.setArg(0, d_bweights[0]);
        kernelUpdate.setArg(1, d_gradB[0]);
        kernelUpdate.setArg(2, firstLayerCols);
        kernelUpdate.setArg(3, inWidth);
        kernelUpdate.setArg(4, learningRate);
        kernelUpdate.setArg(5, LAMBDA_L1);
