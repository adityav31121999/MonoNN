    ...
    try {
        ...
        while (true) {
            ...
            for (int i = 0; i < this->layers; ++i) {
                size_t c_size = cweights[i].size() * cweights[i][0].size();
                size_t b_size = bweights[i].size() * bweights[i][0].size();
                cl::NDRange globalUpdateC = calculate_global_1d(WORKSIZE_1D, c_size);
                cl::NDRange globalUpdateB = calculate_global_1d(WORKSIZE_1D, b_size);
                ...
                CL_CHECK(clCommandQueue.enqueueNDRangeKernel(kernelUpdateWeights, cl::NullRange, globalUpdateC, local_1d));

                kernelUpdateWeights.setArg(0, d_bweights[i]);
                kernelUpdateWeights.setArg(1, d_gradB[i]);
                switch (weightUpdateType) {
                    case 0: kernelUpdateWeights.setArg(2, learningRate); kernelUpdateWeights.setArg(3, (int)b_size); break;
                    case 1: kernelUpdateWeights.setArg(2, (int)b_size); kernelUpdateWeights.setArg(3, learningRate); kernelUpdateWeights.setArg(4, LAMBDA_L1); break;
                    case 2: kernelUpdateWeights.setArg(2, (int)b_size); kernelUpdateWeights.setArg(3, learningRate); kernelUpdateWeights.setArg(4, LAMBDA_L2); break;
                    case 3: kernelUpdateWeights.setArg(2, (int)b_size); kernelUpdateWeights.setArg(3, learningRate); kernelUpdateWeights.setArg(4, LAMBDA_L1); kernelUpdateWeights.setArg(5, LAMBDA_L2); break;
                    case 4: kernelUpdateWeights.setArg(2, (int)b_size); kernelUpdateWeights.setArg(3, learningRate); kernelUpdateWeights.setArg(4, WEIGHT_DECAY); break;
                    case 5:
                ...
                CL_CHECK(clCommandQueue.enqueueNDRangeKernel(kernelUpdateWeights, cl::NullRange, globalUpdateB, local_1d));
        ...
        std::cerr << "Error during clBufTrainBatch (mnn): " << e.what() << std::endl;
    }
...
void mnn2d::clBufTrainBatch(const std::vector<std::vector<std::vector<float>>>& inputs, const std::vector<std::vector<float>>& targets) {
    ...
    // --- Buffer Allocation ---
    cl::Buffer d_input_batch, d_target_batch, d_final_output;
    std::vector<cl::Buffer> d_cweights(layers), d_bweights(layers), d_gradC(layers), d_gradB(layers);
    std::vector<cl::Buffer> d_dotProds(layers), d_activate(layers);
    cl::Buffer d_totalCgrad_buf, d_totalBgrad_buf;
    ...
    try {
        ...
        d_totalCgrad_buf = cl::Buffer(clContext, CL_MEM_READ_WRITE, max_total_grad_size * batchSize * sizeof(float)); CL_CHECK(err);
        d_totalBgrad_buf = cl::Buffer(clContext, CL_MEM_READ_WRITE, max_total_grad_size * batchSize * sizeof(float)); CL_CHECK(err);
        d_grad_x_CT_buf = cl::Buffer(clContext, CL_MEM_READ_WRITE, max_act_size * sizeof(float)); CL_CHECK(err);
        d_dprev_p_buf = cl::Buffer(clContext, CL_MEM_READ_WRITE, max_act_size * sizeof(float)); CL_CHECK(err);
        d_dprev_act_buf = cl::Buffer(clContext, CL_MEM_READ_WRITE, max_act_size * sizeof(float)); CL_CHECK(err);
        ...
        while (true) {
            ...
            // --- Forward Propagation (adapted from mnn2d::clForprop(batch)) ---
            cl::Buffer d_current_act = d_input_batch;
            cl::Kernel kernelForwardBatch = kernels.at("kernelLayerForwardBatch4");
            ...
            kernelForwardBatch.setArg(3, d_bweights[0]);
            kernelForwardBatch.setArg(4, (int)batchSize);
            kernelForwardBatch.setArg(5, (int)single_input_height);
            kernelForwardBatch.setArg(6, (int)single_input_width);
            kernelForwardBatch.setArg(7, (int)width[0]);
            kernelForwardBatch.setArg(8, order);
            ...
            for (int i = 1; i < layers; ++i) {
                ...
                kernelForwardBatch.setArg(3, d_bweights[i]);
                kernelForwardBatch.setArg(4, (int)batchSize);
                kernelForwardBatch.setArg(5, (int)inHeight);
                kernelForwardBatch.setArg(6, (int)width[i - 1]);
                kernelForwardBatch.setArg(7, (int)width[i]);
                kernelForwardBatch.setArg(8, order);
            ...
            for(int i = 0; i < layers; ++i) {
                ...
                for(int j = 0; j < batchSize; ++j) {
                    ...
                    d_incoming_per_batch[i][j] = cl::Buffer(clContext, CL_MEM_READ_WRITE, act_size * sizeof(float)); CL_CHECK(err);
                    d_dotProds_per_batch[i][j] = cl::Buffer(clContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, act_size * sizeof(float), (void*)flatten(dotBatch[i][j]).data(), &err); CL_CHECK(err);
                    d_activate_per_batch[i][j] = cl::Buffer(clContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, act_size * sizeof(float), (void*)flatten(actBatch[i][j]).data(), &err); CL_CHECK(err);
                }
            }

            // error gradient
            std::vector<std::vector<float>> acts = reshape(final_output_flat, batchSize, outSize);
            for (int i = 0; i < batchSize; ++i) {
                d_err_per_batch[i] = cl::Buffer(clContext, CL_MEM_READ_WRITE, targets[i].size() * sizeof(float)); CL_CHECK(err);
                cl::Buffer d_out_single(clContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, outSize * sizeof(float), acts[i].data(), &err); CL_CHECK(err);
                cl::Buffer d_exp_single(clContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, targets[i].size() * sizeof(float), (void*)targets[i].data(), &err);
                CL_CHECK(err);
                ...
                for(size_t r = 0; r < last_layer_rows; ++r) {
                    CL_CHECK(clCommandQueue.enqueueCopyBuffer(d_err_per_batch[i], d_incoming_per_batch[layers-1][i], 0, r * last_layer_cols * sizeof(float), sizeof(float) * last_layer_cols));
                }
                d_out_single = cl::Buffer(); d_exp_single = cl::Buffer();
            }
            CL_CHECK(clCommandQueue.finish());
            ...
            for (int layer = layers - 1; layer >= 1; --layer) {
                ...
                for (int i = 0; i < batchSize; ++i) {
                    ...
                    kernelMatMul.setArg(0, d_incoming_per_batch[layer][i]);
                    ...
                    kernelHadamard2.setArg(3, d_incoming_per_batch[layer-1][i]);
                    ...
                    kernelPower.setArg(0, d_activate_per_batch[layer-1][i]);
                    ...
                    kernelMatMul.setArg(1, d_incoming_per_batch[layer][i]);
            ...
            // First layer
            int in_h = inputs[0].size();
            int in_w = inputs[0][0].size();
            int first_layer_cols = width[0];
            size_t cweight_flat_size_first = in_h * in_w * first_layer_cols;
            cl::NDRange globalWeightGradFirst = calculate_global_1d(WORKSIZE_1D, cweight_flat_size_first);

            for (int i = 0; i < batchSize; ++i) {
                cl::Buffer d_input_single(clContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, in_h * in_w * sizeof(float), (void*)flatten(inputs[i]).data(), &err); CL_CHECK(err);
                cl::Buffer d_input_p(clContext, CL_MEM_READ_WRITE, in_h * in_w * sizeof(float)); CL_CHECK(err);
                kernelPower.setArg(0, d_input_single);
                ...
                kernelMatMul.setArg(1, d_incoming_per_batch[0][i]);
                ...
                d_input_single = cl::Buffer(); d_input_p = cl::Buffer(); d_input_p_T = cl::Buffer(); d_ones_T = cl::Buffer();
            }
            CL_CHECK(clCommandQueue.finish());
            ...
            kernelScale.setArg(2, ALPHA);
            kernelScale.setArg(3, (int)cweight_flat_size_first);
            CL_CHECK(clCommandQueue.enqueueNDRangeKernel(kernelScale, cl::NullRange, calculate_global_1d(WORKSIZE_1D, cweight_flat_size_first), local_1d));
        ...
        std::cerr << "Error during clBufTrainBatch (mnn2d): " << e.what() << std::endl;
    }

    // --- Buffer Cleanup ---
    d_input_batch = cl::Buffer(); d_target_batch = cl::Buffer(); d_final_output = cl::Buffer();
    d_totalCgrad_buf = cl::Buffer(); d_totalBgrad_buf = cl::Buffer();
