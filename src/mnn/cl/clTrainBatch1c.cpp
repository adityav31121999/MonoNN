...
/**
 * @brief trains the mnn2d network on a batch of data for single cycle using OpenCL.
 * @param inputs A vector of input matrices.
 * @param targets A vector of target vectors.
 * @param useBuffer Unused for OpenCL implementation.
 */
void mnn2d::clTrainBatch1c(const std::vector<std::vector<std::vector<float>>>& inputs, const std::vector<std::vector<float>>& targets, bool useBuffer) {
    if (inputs.size() != targets.size()) {
        throw std::invalid_argument("Number of inputs and targets in batch must be the same.");
    }
    if (inputs.empty()) {
    ...
    if (useBuffer == 0) {
        clForprop(inputBatch); // Batch forprop
