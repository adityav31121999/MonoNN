    ...
    try {
        ...
        while (true) {
            ...
            for (int i = 0; i < layers; ++i) {
                ...
                int currentOutWidth = width[i];
                dim3 grid_forward = calculate_grid_2d(currentOutWidth, currentInHeight, WORKSIZE_2D_X, WORKSIZE_2D_Y);
                kernelLayerForward4<<<grid_forward, block_2d>>>(d_current_act, d_dotProds[i], d_cweights[i], d_bweights[i], currentInHeight, currentInWidth, currentOutWidth, order);

                size_t dotprod_size = currentInHeight * currentOutWidth;
                softmax<<<calculate_grid_1d(dotprod_size, WORKSIZE_1D), block_1d>>>(d_dotProds[i], d_activate[i], SOFTMAX_TEMP, dotprod_size);
                d_current_act = d_activate[i];
            }

            meanPool<<<calculate_grid_1d(outWidth, WORKSIZE_1D), block_1d>>>(d_activate[layers - 1], d_out, inHeight, outWidth, 1);
            CU_CHECK(cudaDeviceSynchronize());
            CU_CHECK(cudaMemcpy(output.data(), d_out, output.size() * sizeof(float), cudaMemcpyDeviceToHost));
            ...
            for (int layer = layers - 1; layer >= 1; --layer) {
                ...
                matxmat2mat<<<calculate_grid_2d(prev_cols, curr_rows, WORKSIZE_2D_X, WORKSIZE_2D_Y), block_2d>>>(d_incoming[layer], d_C_T, d_grad_x_CT, curr_rows, curr_cols, prev_cols);

                dPower<<<calculate_grid_1d(prev_act_flat_size, WORKSIZE_1D), block_1d>>>(d_activate[layer - 1], d_dprev_p, order, prev_act_flat_size);
                softmaxDer<<<calculate_grid_1d(prev_act_flat_size, WORKSIZE_1D), block_1d>>>(d_dotProds[layer - 1], d_dprev_act, SOFTMAX_TEMP, prev_act_flat_size);
                hadamard2<<<calculate_grid_1d(prev_act_flat_size, WORKSIZE_1D), block_1d>>>(d_grad_x_CT, d_dprev_p, d_dprev_act, d_incoming[layer - 1], prev_rows, prev_cols);
                ...
                power<<<calculate_grid_1d(prev_act_flat_size, WORKSIZE_1D), block_1d>>>(d_activate[layer - 1], d_prev_p, order, prev_act_flat_size);
                ...
                matxmat2mat<<<calculate_grid_2d(curr_cols, prev_rows, WORKSIZE_2D_X, WORKSIZE_2D_Y), block_2d>>>(d_prev_p_T, d_incoming[layer], d_gradC[layer], prev_cols, prev_rows, curr_cols);
                ...
                matxmat2mat<<<calculate_grid_2d(curr_cols, prev_rows, WORKSIZE_2D_X, WORKSIZE_2D_Y), block_2d>>>(d_onesT, d_incoming[layer], d_gradB[layer], prev_cols, prev_rows, curr_cols);
            ...
            power<<<calculate_grid_1d(input_flat_size, WORKSIZE_1D), block_1d>>>(d_in, d_input_p, order, input_flat_size);
            transpose<<<calculate_grid_2d(in_w, in_h, WORKSIZE_2D_X, WORKSIZE_2D_Y), block_2d>>>(d_input_p, d_input_p_T, in_h, in_w);

            matxmat2mat<<<calculate_grid_2d(first_layer_cols, in_h, WORKSIZE_2D_X, WORKSIZE_2D_Y), block_2d>>>(d_input_p_T, d_incoming[0], d_gradC[0], in_w, in_h, first_layer_cols);
            ...
            matxmat2mat<<<calculate_grid_2d(first_layer_cols, in_h, WORKSIZE_2D_X, WORKSIZE_2D_Y), block_2d>>>(d_onesT_first, d_incoming[0], d_gradB[0], in_w, in_h, first_layer_cols);
