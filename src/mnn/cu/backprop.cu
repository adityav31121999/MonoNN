...
void mnn::cuBackprop(const std::vector<float>& expected) {
    try {
        ...
        CUDA_CHECK(cudaMalloc((void**)&d_in, sizeof(float) * inputSize));
        CUDA_CHECK(cudaMalloc((void**)&d_exp, sizeof(float) * outputSize));
        CUDA_CHECK(cudaMalloc((void**)&d_out, sizeof(float) * outputSize));
        CUDA_CHECK(cudaMalloc((void**)&d_err, sizeof(float) * outputSize));
        ...
        for(int i = 0; i < this->layers; i++) {
            ...
            CUDA_CHECK(cudaMalloc((void**)&d_cweights[i], sizeof(float) * cweight_size));
            CUDA_CHECK(cudaMalloc((void**)&d_bweights[i], sizeof(float) * bweight_size));
            ...
            CUDA_CHECK(cudaMalloc((void**)&d_gradC[i], sizeof(float) * cweight_size));
            CUDA_CHECK(cudaMalloc((void**)&d_gradB[i], sizeof(float) * bweight_size));
            CUDA_CHECK(cudaMalloc((void**)&d_activate[i], sizeof(float) * act_size));
            CUDA_CHECK(cudaMalloc((void**)&d_dotProds[i], sizeof(float) * act_size));
            ...
            CUDA_CHECK(cudaMalloc((void**)&d_incoming[i], sizeof(float) * act_size));
        }

        for(int i = 0; i < this->layers - 1; i++) {
            size_t act_size = activate[i].size();
            CUDA_CHECK(cudaMalloc((void**)&d_dpow[i], sizeof(float) * act_size));
            CUDA_CHECK(cudaMalloc((void**)&d_dact[i], sizeof(float) * act_size));
        }
        ...
        if (max_outgoing_size > 0) {
            CUDA_CHECK(cudaMalloc((void**)&d_preoutgoing_l, sizeof(float) * max_outgoing_size));
            CUDA_CHECK(cudaMalloc((void**)&d_outgoing_l, sizeof(float) * max_outgoing_size));
        }
        ...
        CUDA_CHECK(cudaMalloc((void**)&d_ones, sizeof(float) * max_layer_width));
        ...
        for(int layer = layers - 1; layer >= 1; layer--) {
            ...
            float *d_C_T = nullptr;
            CUDA_CHECK(cudaMalloc((void**)&d_C_T, sizeof(float) * cweight_flat_size));
...
void mnn2d::cuBackprop(const std::vector<float>& expected) {
    try {
        ...
        float *d_in = nullptr, *d_exp = nullptr, *d_out = nullptr, *d_err = nullptr, *d_ones = nullptr, *d_onesT = nullptr;
        ...
        size_t outputSize = output.size();
        CUDA_CHECK(cudaMalloc((void**)&d_in, sizeof(float) * input_flat_size));
        CUDA_CHECK(cudaMalloc((void**)&d_out, sizeof(float) * outputSize));
        CUDA_CHECK(cudaMalloc((void**)&d_exp, sizeof(float) * outputSize));
        CUDA_CHECK(cudaMalloc((void**)&d_err, sizeof(float) * outputSize));
        ...
        for (int i = 0; i < this->layers; ++i) {
            ...
            max_act_size = std::max(max_act_size, dot_size);

            CUDA_CHECK(cudaMalloc((void**)&d_cweights[i], c_size * sizeof(float)));
            CUDA_CHECK(cudaMalloc((void**)&d_bweights[i], b_size * sizeof(float)));
            CUDA_CHECK(cudaMalloc((void**)&d_dotProds[i], dot_size * sizeof(float)));
            CUDA_CHECK(cudaMalloc((void**)&d_activate[i], dot_size * sizeof(float)));
            CUDA_CHECK(cudaMalloc((void**)&d_gradC[i], c_size * sizeof(float)));
            CUDA_CHECK(cudaMalloc((void**)&d_gradB[i], b_size * sizeof(float)));
            CUDA_CHECK(cudaMalloc((void**)&d_incoming[i], dot_size * sizeof(float)));
        ...
        for (int i = 0; i < this->layers - 1; ++i) {
            ...
            CUDA_CHECK(cudaMalloc((void**)&d_outgoing[i], act_size * sizeof(float)));
            CUDA_CHECK(cudaMalloc((void**)&d_dpow[i], act_size * sizeof(float)));
            CUDA_CHECK(cudaMalloc((void**)&d_dact[i], act_size * sizeof(float)));
        }

        if (max_act_size > 0) {
            CUDA_CHECK(cudaMalloc((void**)&d_grad_x_CT, max_act_size * sizeof(float)));
            CUDA_CHECK(cudaMalloc((void**)&d_dprev_p, max_act_size * sizeof(float)));
            CUDA_CHECK(cudaMalloc((void**)&d_dprev_act, max_act_size * sizeof(float)));
        }
        ...
        for(int r = 0; r < last_rows; ++r) {
            for(int c = 0; c < last_cols; ++c) {
                last_layer_err[r * last_cols + c] = out_err_host[c] / (float)last_rows; // Corrected: Error should be distributed (dL/da_i = dL/dPool * dPool/da_i = dL/dPool * 1/N)
            }
        }
        ...
        for (int layer = layers - 1; layer >= 1; --layer) {
            ...
            dPower<<<grid1D_prev, block_1d>>>(d_activate[layer-1], d_dprev_p, order, (int)prev_act_size);

            // d(prev_act) (Softmax Derivative)
            // if (prev_act_size > 256) throw std::runtime_error("Softmax derivative size too large.");
            softmaxDer<<<dim3(1), dim3(prev_act_size)>>>(d_dotProds[layer-1], d_dprev_act, SOFTMAX_TEMP, (int)prev_act_size);

            // outgoing = (dL/dz * C^T) . d(prev_p) . d(prev_act)
            hadamard2<<<grid1D_prev, block_1d>>>(
                d_grad_x_CT, d_dprev_p, d_dprev_act, d_outgoing[layer-1], prev_rows, prev_cols
            );

            // a_prev -> a_prev^n
            float *d_prev_p = nullptr;
            CUDA_CHECK(cudaMalloc((void**)&d_prev_p, sizeof(float) * prev_act_size));
            ...
            float *d_prev_p_T = nullptr;
            size_t prev_p_T_size = prev_cols * prev_rows;
            CUDA_CHECK(cudaMalloc((void**)&d_prev_p_T, sizeof(float) * prev_p_T_size));
            ...
            std::vector<float> ones(prev_act_size, 1.0f);
            CUDA_CHECK(cudaMalloc((void**)&d_ones, sizeof(float) * prev_act_size));
            CUDA_CHECK(cudaMemcpy(d_ones, ones.data(), sizeof(float) * prev_act_size, cudaMemcpyHostToDevice));
            CUDA_CHECK(cudaMalloc((void**)&d_onesT, sizeof(float) * prev_p_T_size));
            transpose<<<calculate_grid_2d(prev_cols, prev_rows, CUDA_BLOCK_SIZE_2D_X, CUDA_BLOCK_SIZE_2D_Y), block_2d>>>(
            ...
            matxmat2mat<<<gridMatMul, block_2d>>>(
                ...
                d_incoming[layer],
                d_gradB[layer],
                prev_cols, // mat1Rows (d_onesT rows)
                prev_rows, // mat1Cols (d_onesT cols) == mat2Rows (d_incoming rows)
                curr_cols  // mat2cols (d_incoming cols)
            );
            ...
            scaleByValue<<<calculate_grid_1d(bweight_flat_size, CUDA_BLOCK_SIZE_1D), block_1d>>>(
                d_gradB[layer], 
                d_gradB[layer], 
                1.0f - alpha, (int)bweight_flat_size
            );
        ...
        CUDA_CHECK(cudaMalloc((void**)&d_input_p_T, sizeof(float) * in_p_T_size));
        ...
        CUDA_CHECK(cudaMalloc((void**)&d_ones, sizeof(float) * in_flat_size));
        CUDA_CHECK(cudaMemcpy(d_ones, ones_L0.data(), sizeof(float) * in_flat_size, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMalloc((void**)&d_onesT, sizeof(float) * in_p_T_size));
        ...
        matxmat2mat<<<gridMatMul, block_2d>>>(
            ...
            d_incoming[0],
            d_gradB[0],
            inWidth,    // mat1Rows (d_onesT rows)
            inHeight,   // mat1Cols (d_onesT cols) == mat2Rows (d_incoming rows)
            firstLayerCols // mat2cols (d_incoming cols)
        );
        CUDA_CHECK(cudaFree(d_onesT));
        ...
        // free memory
        CUDA_CHECK(cudaFree(d_in)); CUDA_CHECK(cudaFree(d_out)); CUDA_CHECK(cudaFree(d_exp)); CUDA_CHECK(cudaFree(d_err));
