 ...
 */
void mnn1d::testOnAllWeights(std::string dataSetPath, bool isRGB, bool useThreadOrBuffer)
{
    // first find all .bin files in the path
    int totalBinFiles = 0;
    std::vector<std::string> binFiles;
    std::string weightsPath = dataSetPath + "/mnn";
    ...
    std::string path2all = dataSetPath + "/mnn1d/allTests";
    try {
    ...
    std::cout << "\n--- Starting All Weights Test (mnn1d) ---" << std::endl;
    ...
    std::string synopsisFilePath = dataSetPath + "/mnn1d/allTests/allWeightsTestSynopsis.csv";
    ...
    std::cout << "--- Test For All Weights Finished (mnn1d) ---" << std::endl;
}