...
std::vector<float> multiply(const std::vector<float>& a, const std::vector<float>& b) {
    if (a.size() != b.size()) {
        throw std::invalid_argument("Incompatible dimensions for vector element-wise multiplication");
    }
    std::vector<float> result(a.size());
    for (size_t i = 0; i < a.size(); ++i) {
        result[i] = a[i] * b[i];
...
/**
 * @brief Multiplies a vector with a matrix (Matrix * Vector logic), used for weighted pooling.
 * Computes the weighted sum of columns for each row.
 */
...
/**
 * @brief Element-wise multiplication of two matrices (Hadamard product).
 */
std::vector<std::vector<float>> multiply(const std::vector<std::vector<float>>& a, const std::vector<std::vector<float>>& b) {
    if (a.size() != b.size() || (a.size() > 0 && a[0].size() != b[0].size())) {
        throw std::invalid_argument("Incompatible dimensions for matrix element-wise multiplication");
    }
    std::vector<std::vector<float>> result(a.size(), std::vector<float>(a.empty() ? 0 : a[0].size()));
    for (size_t i = 0; i < a.size(); ++i) {
        for (size_t j = 0; j < a[0].size(); ++j) {
            result[i][j] = a[i][j] * b[i][j];
        }
