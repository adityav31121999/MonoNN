...
/**
 * @brief Class representing a Dynaic Monomial Neural Network (MNN).
 */
class dyMnn1d {
public:
    ...
    std::vector<float> powers;  // powers for each layer of dyMnn1d
...
// constructors
    ...
    dyMnn1d() = default;
    dyMnn1d(int insize, int outsize, int layers, float order, std::string binFileAddress);
    dyMnn1d(int insize, int outsize, int dim, int layers, float order, std::string binFileAddress);
    dyMnn1d(int insize, int outsize, std::vector<int> width, float order, std::string binFileAddress);
