...
/**
 * @brief Class representing a Monomial Neural Network for 2D input and 1D output 
 * with layer-specific orders (LMNN2D).
 */
class lmnn2d {
public:
    ...
    std::vector<float> orders;      // order for each layer
...
// constructors
    ...
    lmnn2d() = default;
    lmnn2d(int inw, int inh, int outw, int layers, float order, std::string binFileAddress);
    lmnn2d(int inw, int inh, int outw, int dim, int layers, float order, std::string binFileAddress);
    lmnn2d(int inw, int inh, int outw, std::vector<int> width, float order, std::string binFileAddress);
