...
/**
 * @brief Class representing a Monomial Neural Network with layer specific orders
 */
class lmnn1d {
public:
...
// constructors
    ...
    lmnn1d() = default;
    lmnn1d(int insize, int outsize, int layers, float order, std::string binFileAddress);
    lmnn1d(int insize, int outsize, int dim, int layers, float order, std::string binFileAddress);
    lmnn1d(int insize, int outsize, std::vector<int> width, float order, std::string binFileAddress);
